...
RSpec.describe 'Billet' do
  let(:transaction_product) { attributes_for(:transaction_product, price_unit: 0.00) }
  ...
  it { expect(json['payment']['linha_digitavel']).not_to be_empty }
  it { expect(json['payment']['price_payment'].to_f).to eq(price_payment) }
  it { expect(json['payment']['price_original'].to_f).to eq(price_payment) }
